...
result = myfunc2()
# 报错：NameError: name 'x' is not defined因为x是局部变量，在函数内部定义，在函数外部无法访问。
...
# --->520
print(x)
# --->520全局变量被函数内部的global()强制改变了。会造成混乱。
...
str(520)
# --->>TypeError: 'str' object is not callable, 因为str被定义为变量名，而不是函数了。
# str原来的作用是把数值转换成字符串，现在被定义为变量名，所以不能再调用它。
