...
print("-" * 40)
print(x.partition("."))
# ---->('www','.','inthehome.com') 参数是. 原字符串中与参数匹配的分割成三元组。

print(x.rpartition("."))
# ---->('www.inthehome','.','com')从右往左分割成三元组。)

print(x.partition("ww"))
# ---->('', 'ww', 'inthehome.com')
...
z = "朝辞白帝彩云间\n千里江陵一日还\n两岸猿声啼不住\n轻舟已过万重山"
print(z.splitlines())
# 参数为空，默认按照换行符切割并换行输出一个列表。
print(z.splitlines(True))
# 参数为True,会显示换行符。默认是False，不显示。

# join(iterable)方法：把可迭代对象中的元素用字符串连接起来，并返回连接后的字符串。拼接。

print(".".join(["www", "ilovefish", "com"]))
# 要求是可迭代的对象。输出：www.ilovefish.com。 .是分隔符。
print("^".join(("I", "love", "python")))
# 元组也是可迭代的。输出：I^love^python
