...
from pathlib import (
    Path,
)  # 表示从模块pathlib里面，单独导入Path。这种导入方式，后面引用的时候，不需要前缀模块名。
...
)  # 路径必须加引号，否则python会以为是一个变量。必须为字符串格式。
print(
    p
)  # --->d:/python_learning/fish#这个路径对象是可以打印的，输出的时候，会打印成字符串模式。
...
print(
    q.parents
)  # ---><WindowsPath.parents> 是 pathlib 中一个特殊的对象，表示所有父级目录的集合！
...
print(
    q.stat
)  # ---><bound method Path.stat of WindowsPath('d:/python_learning/fish/fishc_python_test')>
...
print(
    Path("./fishc_python_test").resolve()
)  # -->d:/python_learing/fish/fishc_python_test
print(Path("../fish_pythn_test").resolve())  # -->d:pythn_learning/fish_pythn_test

# 用iterdir()得到该目录下所有的文件和文件夹
print(
    p.iterdir()
)  # --><generator object Path.iterdir at 0x00000264AEC23510>得到一个可迭代生成器
