...
print("today is monthday".startswith("monthday"))
# ---->False
...
print("today is monthday".endswith("monthday"))
# ---->True
print(x.endswith("py", 0, 4))
# ---->True:从0到4下标（不含4），里面以py结尾。

x = "他爱python"
if x.startswith(("你", "我", "他")):
    # 元组里有多个待匹配字符串，有一个匹配成功就可以。
    print("我们都爱python")
...
# startswith()  用于判断 子字符串是否处于字符串的起始位置
print("today is monthday".startswith("today"))
# ---->True

# isupper()     用于判断 字符串是否是大写
print("today is monthday".isupper())
# ---->False

# islower()     用于判断 字符串是否全部是小写
print("today is monthday".islower())
# ---->True


# istitle()     用于判断 每个单词的首字母大写，其余字母全小写，且字符串非空.这叫： 标题模式。
print("today is monthday".istitle())
# ---->False  每个单词的首字母不是大写


# isalpha()     用于判断 字符串是否全部由字母组成，且不能包含空格，符号，数字
print("today is monthday".isalpha())
# ---->False   字符串全是字母，但是有空格,空格不是字母。

print("-" * 50)


# isascii()     用于判断 字符串是否全部由ASCII组成 字符串中所有字符的 ASCII 编码在 0-127 范围内，就返回 True。
print("how are your sun 12 years Bob?".isascii())
# ---->True
...
print("hulllaaa".isnumeric())
# ---->False 必须是数值，如:三 ，44，four
print("三four44".isnumeric())
# ---->False four英文数不行
print("三99".isnumeric())
# ---->True
"""
...
# isidentifier()用于判读 字符串是否由 PYTHON的合法的标识符组成
print("_code123H/*%W".isidentifier())
# ---->False  与python 文件名命名规则类似。

print("I_love_520".isidentifier())
# ---->True
print("520_I_love".isidentifier())
# ---->False
...
# 判断一个字符串是不是python的保留字符串。要先导入keyword模块。
