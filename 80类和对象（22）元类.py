...
#当你执行 c = C() 时，Python 会调用元类 MetaC 的 __call__ 方法。
# 你的 MetaC.__call__ 代码是：
# def __call__(cls, *args, **kwargs):
    # print("__call__() in MetaC lalala ")
# 它只打印了一行文字。
# 它没有调用 type.__call__(cls, *args, **kwargs)。
# 它没有 return 任何东西（默认返回 None）。

# 你的 MetaC.__call__ 没有调用 type.__call__，所以它没有执行实际的实例化过程，只是打印了一行文字。
# 它也不需要 return，因为它返回的是 None（默认）。
# 如果你想要 c 成为一个真正的实例，就必须调用并返回 type.__call__(cls, *args, **kwargs)。


# 所以 c 得到的是 None，而不是一个 C 的实例。
# 没有 return type.__call__()，c 就是 None。
